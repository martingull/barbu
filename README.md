...
- A Black Lady table with the same Learn, Practice, Play, and Perfect structure; Play starts with pass-three-left and then moves into a local multi-hand match to 50 points with 2C opening, first-trick penalty restrictions, hearts-broken lead restrictions, queen-danger scoring, and shoot-the-moon scoring, while Practice trains hearts and queen-danger patterns on the shared trick-taking surface.
...
- **Black Lady**: the familiar Queen-of-Spades starter game that proves the shared Hearts-family engine can support more than Barbu.

Black Lady should be real but intentionally focused: pass three cards left, then play local hands on the shared trick-taking table until one seat reaches a 50-point testing target, scoring hearts plus the queen of spades. The current rule target includes the Wikipedia-style holder-of-2C opening, first-trick penalty restrictions, no heart leads until hearts are broken, and shooting the moon as 0 for the shooter and 26 for every other seat. Rotating pass directions and fuller match options are later unless deliberately added.

Whist can remain visible as the next free starter placeholder until Barbu and Black Lady feel stable. Solitaire, Bridge, Gin Rummy, Canasta, and Card Counting can remain visible as future packs or skill areas, but should not distract from getting Barbu and Black Lady playable on the phone.
